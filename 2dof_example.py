...
def plot_circles(r1 = 3, r2 = 12, a = 5, l=6):
    circle = plt.Circle((r1, 0), radius=l, fc='y', fill=False)
    plt.gca().add_patch(circle)
    circle = plt.Circle((r2, 0), radius=l, fc='y', fill=False)
    plt.gca().add_patch(circle)
    circle = plt.Circle((r1, a), radius=l, fc='y', fill=False)
    plt.gca().add_patch(circle)
    circle = plt.Circle((r2, a), radius=l, fc='y', fill=False)
    plt.gca().add_patch(circle)
...
def symbolic_2dof_func(l1=6, l2=6, a=5):
    ...
    v = sym.symbols('v1, v2')
    u = sym.symbols('u1, u2')
    f = sym.Matrix([[-(u[0] - v[0])**2 - u[1]**2 + l1**2],
                    [-(u[0] - v[1])**2 - (u[1] - a)**2 + l2**2]])
    return f, u, v

N = 20  # The number of boxes on uniform grid
##### 2-DOF
f_sym, u_sym, v_sym = symbolic_2dof_func()
v1 = ival.Interval([3, 12.])
v2 = ival.Interval([3, 12.])
v_ival = [v1, v2]
